...
;(function () {
  ...
  */
  ...
  function fenToObj (config) {
    ...
    config.sparePieces = chunk;
    console.log("spare pieces = " + config.sparePieces);
    // cut off any move, castling, etc info from the end
    // we're only interested in position information
    fen = fen.replace(/ .+$/, '')
    var rows = fen.split('/')
  ...
  function expandConfigArgumentShorthand (config) {
    console.log("expandConfigArgumentShorthand()");
    console.log(config);
    if (config === 'start') {
      console.log("config = start");
      config = {position: deepCopy(START_POSITION)}
    } else if (validFen(config)) {
      console.log("validFen");
  ...
  function constructor (containerElOrString, config) {
    ...
    function setInitialState () {
	  ...
	  console.log("validFen");
          currentPosition = fenToObj(config)
    ...
    function buildPieceHTML (piece, hidden, id, count) {
      var html = '';
      if (hidden == false) {
	  html = '<img class="{piece}" src="' + '/static/img/chesspieces/numbers/blank.png' + '" '; 
	  //html = '<img src="' + buildPieceImgSrc(piece) + '" '; ///img/chesspieces/numbers/blank.png" ';
      } else {
	  html = '<img class={piece} src="' + buildPieceImgSrc(piece) + '" ';
      }
      if (isString(id) && id !== '') {
        html += 'id="' + id + '" '
      }
      ...
      if (hidden) {
        ...
        html += 'background-image:url(/static/img/chesspieces/numbers/blank-' + count + '.png),url(/static/img/chesspieces/wikipedia/' + piece + '.png);';
    ...
    function buildSparePiecesHTML (pieces) {

         
      var html = ''
      for (const [key, value] of Object.entries(pieces)) {
        html += buildPieceHTML(key, false, sparePiecesElsIds[key], value)
      }
	...
	var blackPieces = {};
	var whitePieces = {};
	for (var i = 0; i < config.sparePieces.length; i++) {
          var c = config.sparePieces.charAt(i);
          if (c == c.toUpperCase()) {
	    if (isNaN(whitePieces['w' + c])) whitePieces['w' + c] = 0;
	    whitePieces['w' + c] += 1;
	  } else {
	    if (isNaN(blackPieces['b' + c.toUpperCase()])) blackPieces['b' + c.toUpperCase()] = 0;
            blackPieces['b' + c.toUpperCase()] += 1;
	  }
	}
	console.log(blackPieces);
	$sparePiecesTop.html(buildSparePiecesHTML(blackPieces))
	$sparePiecesBottom.html(buildSparePiecesHTML(whitePieces))
